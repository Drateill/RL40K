...
class EnvironmentRenderer:
    ...
    def __init__(self, screen_width: int, screen_height: int):
      ...
      # ✅ NOUVEAU : Listes d'éléments fixes pour éviter le flickering
        self.floor_decorations = []
        self.current_environment = None
        self.decorations_generated = False
    
    def generate_floor_decorations(self, environment_system, world_width: int, world_height: int):
        """✅ NOUVEAU : Génère une fois les décorations au sol pour éviter le flickering"""
        if self.decorations_generated and self.current_environment == environment_system.current_environment:
            return  # Déjà généré pour cet environnement
        
        self.floor_decorations.clear()
        self.current_environment = environment_system.current_environment
        env_type = environment_system.current_environment
        
        # Générer selon l'environnement (UNE SEULE FOIS)
        if env_type == EnvironmentType.SHIP:
            # Plaques métalliques fixes
            for _ in range(20):  # Nombre fixe
                x = random.randint(100, world_width - 100)
                y = random.randint(100, world_height - 100)
                size = random.randint(20, 40)
                self.floor_decorations.append({
                    'type': 'metal_plate',
                    'x': x, 'y': y, 'size': size,
                    'color': (80, 80, 100)
                })
            ...
            # Taches et grilles fixes
            for _ in range(40):
                x = random.randint(100, world_width - 100)
                y = random.randint(100, world_height - 100)
                size = random.randint(15, 30)
                stain_type = random.choice(['oil', 'rust'])
                color = (40, 30, 20) if stain_type == 'oil' else (80, 40, 20)
                self.floor_decorations.append({
                    'type': 'stain',
                    'x': x, 'y': y, 'size': size,
                    'color': color
        ...
        print(f"✅ Décorations du sol générées : {len(self.floor_decorations)} éléments")
    
    def _draw_floor_pattern(self, screen: pygame.Surface, camera, environment_system: EnvironmentSystem):
        """✅ VERSION CORRIGÉE : Dessine les motifs fixes du sol"""
        
        # S'assurer que les décorations sont générées
        if not self.decorations_generated:
            # Générer avec des dimensions par défaut si pas encore fait
            self.generate_floor_decorations(environment_system, 2048, 1536)
        
        env_type = environment_system.current_environment
        
        # Dessiner la grille de base (lignes fixes)
        self._draw_base_grid(screen, camera, env_type)
        
        # Dessiner les décorations fixes (pas d'aléatoire par frame!)
        for decoration in self.floor_decorations:
            decoration_screen_x, decoration_screen_y = camera.apply_pos(decoration['x'], decoration['y'])
            
            # Vérifier si visible à l'écran
            if (-50 <= decoration_screen_x <= self.screen_width + 50 and 
                -50 <= decoration_screen_y <= self.screen_height + 50):
                
                self._draw_decoration(screen, decoration, decoration_screen_x, decoration_screen_y)    
    ...
    def _draw_base_grid(self, screen: pygame.Surface, camera, env_type):
            """Dessine la grille de base selon l'environnement"""
            if env_type == EnvironmentType.SHIP:
                grid_size = 64
                line_color = (60, 60, 80)
            elif env_type == EnvironmentType.TEMPLE:
                ...
                return  # Pas de grille pour les autres
            ...
            for x in range(start_x, start_x + self.screen_width + grid_size, grid_size):
                ...
                if 0 <= screen_x <= self.screen_width:
                    pygame.draw.line(screen, line_color, (screen_x, 0), (screen_x, self.screen_height), 1)
            
            # Lignes horizontales
            for y in range(start_y, start_y + self.screen_height + grid_size, grid_size):
                screen_y = y - camera.y
                if 0 <= screen_y <= self.screen_height:
                    pygame.draw.line(screen, line_color, (0, screen_y), (self.screen_width, screen_y), 1)

    def _draw_decoration(self, screen: pygame.Surface, decoration: dict, screen_x: int, screen_y: int):
            """Dessine une décoration spécifique"""
            decoration_type = decoration['type']
            color = decoration['color']
            
            if decoration_type == 'metal_plate':
                size = decoration['size']
                pygame.draw.rect(screen, color, (screen_x, screen_y, size, size))
                
            elif decoration_type == 'gold_cross':
                cross_size = 8
            ...
            elif decoration_type == 'stain':
                size = decoration['size']
                pygame.draw.circle(screen, color, (screen_x, screen_y), size)
                
            elif decoration_type == 'fracture':
